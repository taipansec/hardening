...
# function for not repeating same calls
def fsmount(cmd, cmd2):
    return_value = list()
    success, error = check(cmd)

    if success:
        pass
    else:
        success, error = check(cmd2)
...
def _1_1_2_ubu():
    return_value = list()
    success, _ = fsmount('findmnt -n /tmp', 'systemctl is-enabled tmp.mount')
    if success:
...
def _1_1_3_ubu():
    return_value = list()
    success, error = fsmount('findmnt -n /tmp', 'systemctl is-enabled tmp.mount')
    if success:
